...
uname -s: Linux
uname -m: x86_64
PWD: /home/rocky/zenable-labs/labs/mcp-get-started
...
$ TASK=$(jq -nc --arg id "$TASK_ID" '{taskId:$id}')
$ echo "submitted $TASK_ID"
submitted zjIBgKwtLIGMeBZuHTWZMGZJb_hTEIJGOwXhxMjdpaE
$ mcp tasks/get "$TASK_ID" "$TASK" | jq -c '.result | {status, pollIntervalMs}'
{"status":"working","pollIntervalMs":5000}
$ sleep 12
$ mcp tasks/get "$TASK_ID" "$TASK" | jq -c '.result | {status, result: .result.structuredContent}'
...
Deleted: sha256:fba11a3bfe8839bd4c0a18f4d79634b80193f8eeb3374633907a76b66ae03c00
```
