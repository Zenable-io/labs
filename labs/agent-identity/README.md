...
Realm ready: http://localhost:8080/realms/agent-identity/.well-known/openid-configuration
```
...
```console
Agent's DPoP public key thumbprint (RFC 7638): vXNg5qax2nM5ETxrcu37wZiqSmDhXU_cH1hzK54EvS4
...
DPoP -- attacker has exfiltrated the access token
  ...
  [   HELD] captured token + proof, replayed verbatim
            first 200, replay HTTP 401 DPoP proof rejected: replayed jti 9a2745cf-2388-4e48-ac59-7ef290c5298c
...
22/22 cases behaved as documented
```
...
verifier cannot open: 8 digests
```
...
Now look at the digest counts: 9 and 8 unopenable digests for a credential with 7 disclosable claims. The arithmetic doesn't work, and that's the feature.
